...
class SqaStage2(models.Model):
    ...
    def action_check(self):
        ...
        if not employee or employee.sqa_role != 's2':
            raise UserError(_('Only users with SQA Role = S2 (Partial Review) can perform this action.'))
        if not self.sqa_stage2_line_ids:
            raise ValidationError(_('Please verify BOM lines before checking.'))
        stage3 = self.env['sqa.stage3'].create(self._prepare_stage3_vals())
        self.stage3_id = stage3.id
        self.state = 'checked'
        self.message_post(body=_('Stage 2 completed. Stage 3 created: %s') % stage3.name)
        return {
    ...
    def _prepare_stage3_vals(self):
        ...
        for line in self.sqa_stage2_line_ids:
            ...
            line_vals.append((0, 0, {
                ...
                'sqa_stage3_bom_line_ids': bom_vals,
            }))
        return {
...
class SqaStage2Line(models.Model):
    ...
    stage2_id = fields.Many2one('sqa.stage2', string='Stage 2', required=True, ondelete='cascade')
    ...
    sqa_stage2_bom_id = fields.Many2one(
        'sqa.stage2.bom', string='BOM Detail', ondelete='cascade'
    )
    ...
    def _onchange_bom_id(self):
        ...
        self.sqa_stage2_bom_id = self.env['sqa.stage2.bom'].new({
...
class SqaStage2Bom(models.Model):
    ...
    _description = 'SQA Stage 2 BOM (Wizard Parent)'

    stage2_line_id = fields.Many2one('sqa.stage2.line', string='Stage 2 Line', ondelete='cascade')
...
class SqaStage2BomLine(models.Model):
    ...
    stage2_bom_id = fields.Many2one(
        'sqa.stage2.bom', string='Stage 2 BOM', required=True, ondelete='cascade'
    )
    ...
    def write(self, vals):
        ...
        if 'product_qty' in vals:
            ...
            for rec in self:
                ...
                if not bom_line and rec.stage2_bom_id.bom_id:
                    bom_line = self.env['mrp.bom.line'].search([
                        ('bom_id', '=', rec.stage2_bom_id.bom_id.id),
                ...
                if bom_line:
                    ...
                    bom_line.bom_id.message_post(
                        body=_(
                            'Modified from Stage 2 | Product: [%s] | %s → %s'

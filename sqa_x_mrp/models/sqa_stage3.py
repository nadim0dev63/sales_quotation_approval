...
class SqaStage3(models.Model):
    ...
    def action_check(self):
        ...
        if not employee or employee.sqa_role != 's3':
            raise UserError(_('Only users with SQA Role = S3 (Final Costing) can perform this action.'))
        if not self.sqa_stage3_line_ids:
            raise ValidationError(_('Please verify BOM lines before checking.'))
        self.state = 'checked'
        self.message_post(body=_('Stage 3 completed. Ready for Final Stage review.'))
        return True
...
class SqaStage3Line(models.Model):
    ...
    stage3_id = fields.Many2one('sqa.stage3', string='Stage 3', required=True, ondelete='cascade')
    ...
    sqa_stage3_bom_id = fields.Many2one(
        'sqa.stage3.bom', string='BOM Detail', ondelete='cascade'
    )
    ...
    def _onchange_bom_id(self):
        ...
        self.sqa_stage3_bom_id = self.env['sqa.stage3.bom'].new({
...
class SqaStage3Bom(models.Model):
    ...
    _description = 'SQA Stage 3 BOM (Wizard Parent)'

    stage3_line_id = fields.Many2one('sqa.stage3.line', string='Stage 3 Line', ondelete='cascade')
...
class SqaStage3BomLine(models.Model):
    ...
    stage3_bom_id = fields.Many2one(
        'sqa.stage3.bom', string='Stage 3 BOM', required=True, ondelete='cascade'
    )
    ...
    def write(self, vals):
        ...
        if 'product_qty' in vals:
            ...
            for rec in self:
                ...
                if not bom_line and rec.stage3_bom_id.bom_id:
                    bom_line = self.env['mrp.bom.line'].search([
                        ('bom_id', '=', rec.stage3_bom_id.bom_id.id),
                ...
                if bom_line:
                    ...
                    bom_line.bom_id.message_post(
                        body=_(
                            'Modified from Stage 3 | Product: [%s] | %s → %s'
